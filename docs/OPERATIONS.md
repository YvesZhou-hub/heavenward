...
Deployments are recorded in `DEPLOYMENT.md`, and the step-by-step commands are in `CONTRIBUTING.md` under "For maintainers: releasing and deploying". Every release should also meet these requirements:
...
3. Deploy through the Vercel CLI and inspect the deployment's real final status. A created deployment ID or green local build is not a Ready production result.
...
For a hosted rollback, select only a known tested deployment after a maintainer approves the rollback, verify current origin/assets and repeat persistence smoke checks. Keep evidence for both versions. This document is a procedure, not a claim that any deployment or rollback was performed.
